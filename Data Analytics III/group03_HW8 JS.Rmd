...
```{r}
clusters$centers
write.csv(clusters$centers,"desktop\\clusters.csv", row.names = FALSE)
```
...
**Analyzing the cluster centers, all the columns have similar values across the cluster 1-6 other than 'size' and 'high_med_gp'. Based on the difference of these two fields, we came up with some names of the clusters.**   

**We named cluster 1 'Hipster', because it is the largest and has the highest median gross profit percentage stores with large stores. Also this cluster doesn't do promotions and has the lowest velocity of new products and sells the least of the most common products.**   
**We named cluster 2 'Avg_Joe', because this cluster sells the most of the most common products. However, it has the lowest gross profit margin with lowest size of stores.**
**We named cluster 3 'Violet', because this cluster sells most gums and waters. We thought of Violet from "Charlie and Chocolate Factory."**   
**We named cluster 4 'Sweet_Tooth', because this cluster sells mostly bagged candies. Velocity B unit is the second highest selling group of products.**   
**We named cluster 5 'Gamer', because this cluster sells the highest energy drinks and large bags of potato chips. Also it has the highest velocity of new products. Gamers love new technology and junk foods.**
**We named cluster 6 'Fire_Sale', because this cluster has the highest volume of promotional sales.**
...
**All else is equal for 6 clusters other than this data, there seems to be a relationship between size of the stores and gross profits in general. Promotions and Velocity A (most common products) negatively impact profitability for NANSE based on Cluster 2 & 6. NANSE could further look into what the cluster 1 stores are doing different from cluster 2 & 6 in terms of velocity units.**
...
**Based on the data in 'clustering_input1', Parry Sound seems to be in the cluster 1 and Trenton cluster 2. This means that Parry Sound is one of the large size stores with above high median gross profit while Trenton is one of the small size stores with below high median gross profit. We can prove it by observing the data below. The size for Parry Sound is 955 and high_med_gp 1 and Trenton 351 & 0 for the same columns. It's also notable that other fields seem to vary more between two cities than respective cluster's fields average each city belongs to.**
...
**K-means provided more useful information for this dataset because dataset was more clustered together. K-means was able to parse the data with fixed cluster size where as the dataset was too clustered for DBSCAN. If the dataset had been more divided and clustered, then DBSCAN would've been a great choice.**
